...
exports.songDetails = function(request, response){
		...
		db.all(sql, function(err, rows){
		let song = rows[0]
		let length = song.bars.length
		let temp = song.bars.substring(0, length-2);

		song.individualBars = temp.split(/\|{1,2}/);
		
		song.rendersongs = groupIntoFours(song.individualBars)
