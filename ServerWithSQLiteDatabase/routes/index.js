  ...
  if (!auth) {
    ...
    db.all("SELECT userid, password,role FROM users", function(err, rows) {
      for (var i = 0; i < rows.length; i++) {
        if (rows[i].userid == username & rows[i].password == password ) {authorized = true; retrieved_user_role =rows[i].role;} 
      }
      if (authorized == false) {
...
exports.find = function(request, response) {
  ...
  //console.log("T1 " + urlObj.query['title'].replace(/\s+/g, "%"));
  if (urlObj.query['title']) {
    console.log("inn here");
    //console.log("finding title: " + urlObj.query['title'].replace(/\s+/g, "%"));
    sql = "SELECT id, title FROM songs WHERE title LIKE '%" +
      urlObj.query['title'].replace(/\s+/g, "%") + "%'";
  }
